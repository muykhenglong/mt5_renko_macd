...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 13 14:10:42 2024

MetaTrader5 - Intraday Renko MACD
    ...
    try: 
        ...
        if signal == 'Buy' or signal == 'Sell':
            ...
            if tot_pos > 0: 
                ...
                place_market_order(currency, tot_pos, "Buy")
            print(f'All position closed for {currency}')
            ...
            tot_pos = (open_pos_cur.type * open_pos_cur.volume).sum()
            place_market_order(currency, abs(tot_pos)+pos_size, "Sell")
...
while time.time() <= timeout:
    try:
        print(f"passthrough at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}")
        main()
